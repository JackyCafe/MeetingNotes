...
<body>
	 <div >
		...
		<form  action='notes' method='post' enctype='multipart/form-data'>
			...
			列管事項來源:<textarea rows='10' cols='80' id='souece' name='source' >${param.source}</textarea><p>
			等級: <input type='text' id='grade' name='grade' value="${param.grade}" /><p>
			討論事項:<textarea rows='10' cols='100' id='discuss_matter' name='discuss_matter'>${param.discuss_matter}</textarea><p>
			決議:<textarea rows='20' cols='100' id='presentation' name='presentation'>${param.presentation}</textarea><p>
			參考:<input type='text' id='reference' name='reference' value="${param.reference}" />
			上傳:<input type='file' id='upload' name='upload' value="${param.upload}"/><p>
